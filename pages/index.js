...
function AppInner(){
  ...
  const dailyXPGiven = useRef(false);
  useEffect(()=>{
    if(user && !dailyXPGiven.current){
      dailyXPGiven.current = true;
      getProgress(user.id).then(p=>{setProgress(p);setProgLoading(false);});
      checkAndUpdateStreak(user.id).then(({streak,bonus})=>{
        if(bonus>0){
          addXP(user.id, bonus, `Streak bonus`).then(()=>refreshProfile());
          showToast(`Streak ${streak} days! +${bonus} XP`);
        } else {
          addXP(user.id, XP_REWARDS.daily_login, "Daily login").then(()=>refreshProfile());
          showToast("+20 XP — Welcome back!");
        }
      });
    }
  },[user]);
